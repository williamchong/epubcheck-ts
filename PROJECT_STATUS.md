...
7. **fflate `__proto__` entry loss** — `unzipSync` returns a plain object and assigns each entry onto it by name, so an archive entry named `__proto__` sets that object's prototype instead of being stored. The entry is gone before this code sees it: `Object.keys` never lists it, and a manifest declaring it draws a false `RSC-001` where Java reads the file. Not fixable at the call site — it needs an unzip path that does not key entries on a plain object, which is the same change tracked for decompression-bomb resistance (`unzipSync` inflates every entry eagerly, so a 612 KB archive can reach 726 MB resident where Java stays near baseline). The reciprocal read side — a lookup for `constructor` or `toString` resolving up the prototype chain — is fixed: `ZipReader` keys entries in a `Map`.
